...
- Use `create_html` for HTML reports: write the report as markdown content and the tool renders, styles, saves, and verifies a standalone HTML file. Fall back to `run_report_code` only when the report needs computed data or a custom layout that markdown cannot express. Use raw `execute_python` only for calculations, table normalization, CSV/Excel generation, PDF creation, validation scripts, or non-report transformations.
